 ...
 int getAction(int id){
  ...
  void startup() {
    ...
    add new PoissonBehavior(10000, {
        ...
        def pay = payload.collate(20)
        mac << new ReservationReq(duration: 250*pay.size(),to:controller_address)
        for(p in pay){
            def k = new TxFrameReq(to:controller_address,protocol:33,data:p,shortcircuit:false,type:1)
            phy << k
        }
  ...
  void newFlowProactive(msg){
        ...
        for(def i=0; i<d; i++){
            def flow = []
            def mask = [reply[(i*5) + 0],reply[(i*5) + 1],reply[(i*5) + 2],reply[(i*5) + 3]]
            
            int action = reply[(i*5) + 4]
            ...
            def elegible = []
            for(e in cached_flows){
                if((e[1] == mask[1] || mask[1] == -1)&&(e[2] == mask[2] || mask[2] == -1)&&(e[3] == mask[3] || mask[3] == -1)&&(e[4] == mask[4] || mask[4] == -1)){
                    elegible.add(e)
